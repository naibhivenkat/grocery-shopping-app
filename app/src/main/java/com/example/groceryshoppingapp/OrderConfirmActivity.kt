...
//
//        Log.d("RAZORPAY_DEBUG", "Using Razorpay order_id=$razorpayOrderId")
//
...
class OrderConfirmActivity : AppCompatActivity(), PaymentResultWithDataListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonPlaceOrder.setOnClickListener {
            if (selectedPaymentMethod == "UPI") {
                orderManager.placeOrder(cartItems, shopId!!, "Razorpay", "")  // same logic
            } else {
                orderManager.placeOrder(cartItems, shopId!!, "Cash", "N/A")
            }
        }
    }
    ...
    fun onOrderPlacedSuccess(orderId: String?, razorpayOrderId: String?, paymentMethod: String) {
        ...
        if (paymentMethod == "Razorpay") {
            paymentManager.startRazorpayCheckout(shopName, totalAmount, razorpayOrderId)
        } else {
