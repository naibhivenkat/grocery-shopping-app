...
//                    Toast.makeText(this@ShopKhataListActivity, "Failed to load", Toast.LENGTH_SHORT).show()
//                    return
//                }
//
//                val body = response.body()
//                accounts.clear()
//                body?.accounts?.let { accounts.addAll(it) }
//                adapter.notifyDataSetChanged()
//
//                binding.tvTotalCustomers.text = "Customers: ${accounts.size}"
//
//                // Sum all balances (you can re-add coerceAtLeast if you want only dues)
...
//                Toast.makeText(this@ShopKhataListActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
//            }
//        })
//    }
//
...
//            ) {
//                if (!response.isSuccessful) {
//                    Toast.makeText(this@ShopKhataListActivity, "Failed to create", Toast.LENGTH_SHORT).show()
//                    return
//                }
//
//                Toast.makeText(this@ShopKhataListActivity, "Khata Created Successfully", Toast.LENGTH_SHORT).show()
//                loadAccounts()
//            }
//
...
class ShopKhataListActivity : AppCompatActivity() {
    ...
    private fun loadAccounts() {
        ...
        // ⭐ ADDED — start refreshing animation
        binding.swipeRefresh.isRefreshing = true

        val shopId = SessionManager.getShopId(this)
        if (shopId.isNullOrEmpty()) {
            Toast.makeText(this, "No shopId found", Toast.LENGTH_SHORT).show()
            binding.swipeRefresh.isRefreshing = false   // ⭐ ADDED
            return
        }
        ...
        api.getKhataCustomers(shopId).enqueue(object : Callback<KhataAccountsResponse> {
            override fun onResponse(
                ...
                body?.accounts?.let { list ->
                    list.forEach { acc ->
                        // ⭐ FIX if shop_name missing (same fix as customer side)
